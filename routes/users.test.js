...
const { testJobIds } = require("../routes/_testCommon.js");
...
describe("POST /users", function () {
  ...
  test("works for users: create non-admin by a non-admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("applyToJob", function () {
  ...
  test("not found if no such job", async function () {
    try {
      await User.applyToJob("u1", 999999, "applied");
      fail();
    } catch (err) {
